...
def process_pdf_true_hybrid(
    ...
    for page_data in pages_data:
        ...
        for proposal in deduped:
            ...
            # Apply classification threshold gate (mirrors classify_blocks() logic)
            final_conf = classification["confidence"]["final_confidence"]
            btype = classification["block_type"]
            if final_conf < thresholds.get(btype, 0.0):
                if btype != "text":
                    classification["block_type"] = "text"
                    btype = "text"
                if final_conf < thresholds.get("text", 0.0):
